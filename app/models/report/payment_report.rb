class Report::PaymentReport < Report::YearlyReport
  ...
  def payments
    ...
    @studio.payments.select('MAX(payments.id), COUNT(*) AS payments_count, SUM(payments.payment_amount) AS total, payments.account_id, MAX(DATE(payments.payment_date)) AS payment_date').where(payment_date: start_date...end_date).group('payments.account_id').reorder('')
  end
